...
class HelpCenterFolderNextPrevious(object):
    ...
    def itemRelatives(self, oid):
        ...
        parent_folder = aq_parent(folder)
        folder_position_in_parent = parent_folder.getObjectPosition(folder.getId())
        ...
        if previous == None:
            # check to see if we have a previous manual section in the parent folder
            if (folder_position_in_parent - 1) >= 0:
                prev_brain = catalog(self.buildNextPreviousSectionQuery(parent = parent_folder,
                                                                        position   = folder_position_in_parent - 1,
                                                                        range      = 'max',
                                                                        sort_order = 'reverse'))
        ...
        if next == None:
            # Get the next manual section
            parent_counter = getattr(aq_base(parent_folder), 'objectCount', None)
            if parent_counter is not None:
                parent_count = parent_counter()
            else:
                parent_count = len(parent_folder.objectIds())
                
            # check to see if we have another manual section in the parent folder
            if (folder_position_in_parent +1) < parent_count:
                next_brain = catalog(self.buildNextPreviousSectionQuery(parent = parent_folder,
                                                                        position   = folder_position_in_parent + 1,
                                                                        range      = 'min'))
    ...
    def buildNextPreviousQuery(self, position, range, sort_order = None):
        ...
        # Filters on content
        query['portal_type'] = "HelpCenterReferenceManualPage"
    ...
    def buildNextPreviousSectionQuery(self, parent, position, range, sort_order = None):
        sort_on                  = 'getObjPositionInParent'
        query                    = {}
        query['sort_on']         = sort_on
        query['sort_limit']      = 1
        query['path']            = dict(query = '/'.join(parent.getPhysicalPath()),
                                        depth = 1)
        ...
        # Filters on content
        query['portal_type'] = "HelpCenterReferenceManualSection"
